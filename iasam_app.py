...
@clear_cache_decorator
def run_get_color_change_image(input_image, sel_mask,hex_color):
    global sam_dict
    if sam_dict["mask_image"] is None or sel_mask is None:
        return None
    ...
    _, mask = cv2.threshold(gray_image, 0, 255, cv2.THRESH_BINARY_INV)
    mask = 255 - mask
    
    image_org= np.array(input_image, dtype=np.uint8)
    # image_org=cv2.resize(image_org, (mask.shape[1], mask.shape[0]))
    if image_org.shape[:2] != mask.shape[:2]:
        raise ValueError("Image and mask must have the same dimensions.")

    # Specify the new color using a hex code (e.g., #FF5733 for orange)
    # hex_color = hex_color
    
    rgb_color = np.array([int(hex_color[1:3], 16), int(hex_color[3:5], 16), int(hex_color[5:7], 16)])
    # bgr_color = rgb_color[::-1]  # Reverse the order to convert from RGB to BGR

    # Create a copy of the original image
    result = image_org.copy()

    # Set the opacity (alpha) to 50% (0.5)
    alpha = 0.5

    # Apply the new color with opacity only where the mask is white
    result[mask == 255] = (1 - alpha) * result[mask == 255] + alpha * rgb_color
    ...
    with block as inpaint_anything_interface:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Tab("Color Change", elem_id="color_tab"):
                    hex_color = gr.ColorPicker(label="color")
                    with gr.Row():
                        with gr.Column():
                            with gr.Row():
                                color_btn = gr.Button("Run Color change", elem_id="color_btn", variant="primary")

                    with gr.Row():
                        color_out_image = gr.Image(label="color change image", elem_id="ie_color_out_image", type="pil", image_mode="RGBA", interactive=False)
            ...
            color_btn.click(
                run_get_color_change_image,
                inputs=[input_image, sel_mask, hex_color],
                outputs=[color_out_image])
